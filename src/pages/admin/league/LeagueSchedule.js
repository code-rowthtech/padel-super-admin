...
const LeagueSchedule = () => {
  ...
  const {
    ...
    schedules,
  } = useSelector((state) => state.league);
  const [activeTab, setActiveTab] = useState("");
  const [selectedRound, setSelectedRound] = useState("regularRound");
  console.log(selectedRound, "selectedRoundselectedRoundselectedRound");
  ...
  const availableCategories =
    selectedLeague?.clubs?.[0]?.participationLimit?.categoryLimits || [];
    ...
    schedulesData.forEach((dateGroup) => {
      dateGroup.schedules.forEach((schedule) => {
        const categoryId = availableCategories.find(
          (cat) => cat.categoryType === schedule.categoryType,
        )?._id;
        ...
        schedule.matches.forEach((match, index) => {
          ...
          const transformedMatch = {
            ...
            homeVenue:
              ...
              (schedule.roundType === "final"
                ? match.teamA?.teamName || `Winner ${index * 2 + 1}`
                : null),
            awayVenue:
              match.teamB?.clubType ||
              (schedule.roundType === "final"
                ? match.teamB?.teamName || `Winner ${index * 2 + 2}`
                : null),
  ...
  const { newMatches, existingMatches } = React.useMemo(() => {
    ...
    Object.keys(matchesByCategory).forEach((categoryId) => {
      ...
      if (categoryMatches.length > 0) {
        ...
        const newMatchesList = categoryMatches.filter(
          (match) => !match.isExisting,
        );
        const editedExistingMatches = categoryMatches.filter(
          (match) => match.isExisting,
        );
        ...
        if (editedExistingMatches.length > 0) {
          ...
          editedExistingMatches.forEach((editedMatch) => {
            const index = existingMerged[categoryId].findIndex(
              (m) => m.id === editedMatch.id,
            );
            if (index !== -1) {
  ...
  const isViewMode =
    !currentScheduleInfo.date && schedules && schedules.length > 0;

  // Get current new and existing matches for display
  const currentNewMatches =
    activeTab === "all"
      ? Object.keys(newMatches)
          .filter((key) => key !== "all")
          .flatMap((key) => newMatches[key] || [])
      : newMatches[activeTab] || [];

  const currentExistingMatches =
    activeTab === "all"
      ? Object.keys(existingMatches)
          .filter((key) => key !== "all")
          .flatMap((key) => existingMatches[key] || [])
      : existingMatches[activeTab] || [];

  // Check if there are any new (unsaved) matches
  const hasUnsavedMatches = currentNewMatches.length > 0;

  const fetchPlayersForClub = useCallback(
    async (clubName, categoryType) => {
  ...
  const handleDateChange = (e) => {
    ...
    if (dateValue) {
      ...
      if (
        ...
        currentScheduleInfo.date !== formatted &&
        hasUnsavedMatches
      ) {
  ...
  const handleVenueChange = (newVenue) => {
    ...
    if (
      ...
      currentScheduleInfo.venue !== newVenue &&
      hasUnsavedMatches
    ) {
  ...
  const handleEditMatch = async (match, categoryId) => {
    ...
    if (match.isExisting && match.homeTeam?.players) {
      ...
      const categoryType = availableCategories.find(
        (c) => c._id === activeTab,
      )?.categoryType;
  ...
  const handleUpdateMatch = async (match, categoryType) => {
    ...
    const scheduleId = match.id
      .toString()
      .replace("existing_", "")
      .split("_")[0];
    const matchIndex = parseInt(
      match.id.toString().replace("existing_", "").split("_")[1],
    );
    ...
    const payload = {
      ...
      endTime: convertTo12Hour(
        match.endTime || calculateEndTime(match.time, match.duration || 60),
      ),
      duration: match.duration || 60,
      teamA:
        homePlayers.length >= 2
          ? {
              ...
              players: homePlayers
                .slice(0, 2)
                .map((p) => ({ playerId: p._id, playerName: p.playerName })),
            }
          : match.homeTeam,
      teamB:
        awayPlayers.length >= 2
          ? {
  ...
  const handleEditTimeChange = (matchId, categoryId, time) => {
    ...
    const updated = currentMatches.map((m) =>
      m.id === matchId
        ? { ...m, time, endTime: calculateEndTime(time, m.duration || 60) }
        : m,
    );
    ...
    const updated = currentMatches.map((m) =>
      m.id === matchId
        ? {
            ...
            endTime: calculateEndTime(m.time, parseInt(duration)),
          }
        : m,
    );
    setMatchesByCategory((prev) => ({ ...prev, [categoryId]: updated }));
  ...
  const handleCreateDate = async () => {
    ...
    try {
      ...
      if (selectedRound === "final") {
        ...
        const categoriesToCreate = formCategory.filter(
          (categoryId) => (matchesByCategory[categoryId] || []).length === 0,
        );
        ...
        categoriesToCreate.forEach((categoryId) => {
          ...
          let currentBaseId =
            existingMatches.length > 0
              ? Math.max(...existingMatches.map((m) => m.id)) + 1
              : 1;
        ...
        await Promise.all(
          availableCategories.map(async (category) => {
            ...
            if (isHomeVenue) {
              const players = await fetchPlayersForClub(
                formVenue,
                category.categoryType,
              );
              const activePlayers = (players || []).filter(
                (p) => p.playerStatus === "active",
              );
              if (activePlayers.length === 2) {
                ...
                catMatches = pairs.map((pair, idx) => {
                  ...
                  return {
                    ...baseMatch,
                    id: splitId,
                    homeTeam: { teamName: `Team ${idx + 1}`, players: pair },
                  };
                });
              }
            }
            ...
            if (Object.keys(catPlayers).length > 0)
              newSelectedPlayersByCategory[categoryId] = catPlayers;
          }),
        );
        ...
        if (Object.keys(newSelectedPlayersByCategory).length > 0) {
          setSelectedPlayers((prev) => {
            ...
            Object.keys(newSelectedPlayersByCategory).forEach((catId) => {
              updated[catId] = {
                ...updated[catId],
                ...newSelectedPlayersByCategory[catId],
              };
            });
            return updated;
          });
    ...
    const updatedMatches = currentMatches.map((match) =>
      match.id === matchId
        ? {
            ...match,
            time,
            endTime: calculateEndTime(time, match.duration || 60),
          }
        : match,
    );
    setMatchesByCategory((prev) => ({
    ...
    const updatedMatches = currentMatches.map((match) =>
      match.id === matchId
        ? {
            ...match,
            duration: parseInt(duration),
            endTime: calculateEndTime(match.time, parseInt(duration)),
          }
        : match,
    );
    setMatchesByCategory((prev) => ({
  ...
  const handleAwayVenueChange = async (matchId, awayVenue) => {
    ...
    setMatchesByCategory((prev) => {
      ...
      if (matchExists) {
        ...
        const match = existingMatches[categoryId]?.find(
          (m) => m.id === matchId,
        );
        if (match) {
          return {
    ...
    if (activePlayers.length === 2) {
      setSelectedPlayers((prev) => ({
        ...
        [categoryId]: {
          ...prev[categoryId],
          [`${matchId}_away`]: activePlayers.map((p) => p._id),
        },
      }));
    }
  };
  ...
  const handleHomeVenueChange = async (matchId, homeVenue) => {
    ...
    if (activePlayers.length === 2) {
      setSelectedPlayers((prev) => ({
        ...
        [categoryId]: {
          ...prev[categoryId],
          [`${matchId}_home`]: activePlayers.map((p) => p._id),
        },
      }));
    }
  };
  ...
  const handleSaveSchedule = async () => {
    ...
    const unsavedCategories = availableCategories.filter((cat) => {
      const catMatches = (matchesByCategory[cat._id] || []).filter(
        (m) => !m.isExisting,
      );
      ...
      for (const cat of unsavedCategories) {
        ...
        for (let i = 0; i < catMatches.length; i++) {
          ...
          if (
            ...
            showError(
              `${cat.categoryType} — Match ${i + 1}: Select exactly 2 players for ${m.homeVenue} (Home)`,
            );
            return;
          }
          if (
            ...
            showError(
              `${cat.categoryType} — Match ${i + 1}: Select exactly 2 players for ${m.awayVenue} (Away)`,
            );
            return;
          }
        }
    ...
    const firstUnsavedMatch = (
      matchesByCategory[unsavedCategories[0]._id] || []
    ).find((m) => !m.isExisting);
    if (!firstUnsavedMatch) {
      showError("No matches to save.");
      return;
    }
  ...
  const handleAddMoreRows = () => {
    ...
    const fallbackVenue =
      currentScheduleInfo.venue || (clubs.length > 0 ? clubs[0].name : "");
    ...
    const newMatch = {
      ...
      homeTeam:
        selectedRound === "final" ? { teamName: `Winner ${nextIdx}` } : null,
      awayTeam:
        selectedRound === "final"
          ? { teamName: `Winner ${nextIdx + 1}` }
          : null,
      time: startTime,
      duration: duration,
      endTime: calculateEndTime(startTime, duration),
  ...
  return (
    <Container fluid className="p-4 bg-white" style={{ minHeight: "100vh" }}>
        ...
        <Col>
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
            ...
            <div className="d-flex align-items-center gap-2 flex-nowrap">
              ...
              <button onClick={handleOpenModal} className="export-btn">
        ...
        <Col md={10} style={{ overflowY: "auto" }}>
          ...
          {selectedRound !== "final" && (
            <Nav
              ...
              className="level-tabs border-0 mb-3"
            >
          ...
          {loadingSchedules ? (
            ...
            <>
              <div className="home-team-header my-3 d-flex justify-content-between align-items-center">
                <span
                  style={{ fontWeight: "600", color: "rgba(37, 37, 37, 0.8)" }}
                >
                  Match Schedule
                </span>
                <div className="d-flex align-items-center gap-3">
                  {currentScheduleInfo.date && (
                    <div className="d-flex align-items-center gap-2">
                      ...
                      >
                        {new Date(currentScheduleInfo.date).toLocaleDateString(
                          "en-GB",
                        )}
                      </span>
                    </div>
                  ...
                  <div style={{ marginBottom: "30px" }}>
                    ...
                    <div
                      style={{
                        overflowX: "auto",
                        borderRadius: "8px",
                        border: "2px solid rgba(31, 65, 187, 0.3)",
                      }}
                    >
                      <table
                        style={{
                      ...
                      >
                        ...
                        <tbody>
                          {availableCategories
                            .filter(
                              (cat) =>
                                activeTab === "all" || activeTab === cat._id,
                            )
                            .map((category) => {
                                ...
                                <React.Fragment key={category._id}>
                                  ...
                                  {categoryMatches.map((match, index) => {
                                    const selectedHomePlayers =
                                      getSelectedPlayers(match.id, "home");
                                    const selectedAwayPlayers =
                                      getSelectedPlayers(match.id, "away");
                                    return (
                                      <tr
                                          ...
                                          <div className="d-flex justify-content-center align-items-center gap-2">
                                            {selectedRound === "final" ? (
                                              ...
                                              <CustomClubSelector
                                                ...
                                                onClubSelect={(clubName) =>
                                                  handleAwayVenueChange(
                                                    match.id,
                                                    clubName,
                                                  )
                                                }
                                                availableClubs={getAvailableAwayClubs(
                                                  match.homeVenue,
                                                )}
                                                selectedPlayers={
                                                  selectedAwayPlayers
                        ...
                        <tbody>
                          {selectedRound === "final"
                            ? availableCategories
                                ...
                                .map((category) => {
                                  const rawMatches =
                                    existingMatches[category._id] || [];
                                  const categoryMatches = rawMatches.map(
                                    (m) => {
                                      const edited = (
                                        matchesByCategory[category._id] || []
                                      ).find((em) => em.id === m.id);
                                    ...
                                    <React.Fragment key={category._id}> 
                                      ...
                                      {categoryMatches.length > 0 ? (
                                        ...
                                        <tr>
                                          ...
                                          >
                                            No matches for this category. Click
                                      ...
                                      {categoryMatches.length > 0 ? (
                                        categoryMatches.map((match, index) => {
                                          ...
                                          const homeClub = clubs.find(
                                            (club) =>
                                              club.name === match.homeVenue,
                                          );
                                          const awayClub = clubs.find(
                                            (club) =>
                                              club.name === match.awayVenue,
                                          );
                                          const selectedHomePlayers =
                                            getSelectedPlayers(
                                              match.id,
      ...
      <ScheduleModal
        ...
        alreadyScheduledCategories={availableCategories
          .filter((cat) => (matchesByCategory[cat._id] || []).length > 0)
          .map((cat) => cat._id)}
