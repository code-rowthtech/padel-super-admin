...
const VenueDetails = ({ formData, onNext, updateFormData }) => {
    ...
    const toggleCheckbox = (section, key) => {
        ...
        if (section === 'courtTypes') {
            const hasCourtType = newValue || Object.entries(newData.courtTypes)
                .filter(([k]) => k !== key)
                .some(([_, v]) => v);
        ...
        if (section === 'features') {
            const hasFeature = newValue || Object.entries(newData.features)
                .filter(([k]) => k !== key)
                .some(([_, v]) => v);
    ...
    return (
        <div className='border-top'>
            <Form onSubmit={handleSubmit} noValidate>
                <h5 style={{ fontWeight: 700, color: '#1F2937' }} className='my-3'>Court Details</h5>
                ...
                <Row className="mb-3">
                    ...
                    <Col md>{renderInput('Zip Code', 'zip')}</Col>
