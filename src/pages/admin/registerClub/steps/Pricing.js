...
import { getSlots, createSlot, updatePrice } from '../../../../redux/thunks';
...
const Pricing = () => {
    ...
    const handleSubmit = (e) => {
        ...
        const selectedSlotType = formData.selectedSlots;

        // Validate slot prices exist
        ...
        const slotData = PricingData?.[0]?.courts?.[0]?.slotTimes || [];
        const businessHours = PricingData?.[0]?.courts?.[0]?.businessHours || [];
        ...
        const courtId = PricingData?.[0]?.courts?.[0]?._id;
        if (!courtId) {
            alert('Court ID is missing or not selected.');
            return;
        }
