...
const RegisterClub = () => {
    ...
    const renderStepContent = () => {
        switch (step) {
            ...
            case 2:
                return <Images formData={formData} onNext={goNext} onBack={goBack} updateFormData={updateFormData} />;
            case 3:
                return <Pricing formData={formData} onBack={goBack} updateFormData={updateFormData} />;
            default:
                return null;
        }
    };

    return <ClubRegistrationLayout currentStep={step}>{renderStepContent()}</ClubRegistrationLayout>;
};
