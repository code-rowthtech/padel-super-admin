...
import { addPlayers, createCustomer } from "../../../../redux/admin/openMatches/thunk";
    ...
    dispatch(createCustomer(userData))
      ...
      .then((res) => {
        if (res?.status === 200) {
          ...
          const addTeam = team === "Team A | B" ? mapTeam(selectedTeam) : mapTeam(team);
          
          dispatch(addPlayers({ matchId, playerId: res?.response?._id, team: addTeam }))
            .then((addRes) => {
              if (!addRes?.error) {
                onPlayerAdded();
              }
        ...
        <Form>
          ...
          <div className="mb-3">
            ...
            <Select
              ...
              style={{ boxShadow: "none" }}
              isDisabled={createCustomerLoading}
            />
            {errors.level && (
