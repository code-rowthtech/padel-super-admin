                ...
                <div className="d-flex flex-wrap gap-2 mb-3">
                  {courts?.map((court) => (
                    <button
                      ...
                      onMouseEnter={(e) => e.currentTarget.style.border = "1px solid #3DBE64"}
                      onMouseLeave={(e) => e.currentTarget.style.border = "2px solid #4949491A"}
                    >
                      {court.courtName}
                    </button>
                  ))}
                      ...
                      <>
                        {(() => {
                          ...
                          return filteredSlots?.map(
                            (
                              { slot, isPast, isSelected, isBooked, isAvailable, hasAmount },
                              i
                            ) => {
                              let tooltipText = "";
                              if (!hasAmount) tooltipText = "Amount not available";
                              else if (isBooked) tooltipText = "Booked";
                              else if (isPast) tooltipText = "Cannot book slot in past hours";
                              ...
                              const buttonEl = (
                                <span className="d-inline-block">
                                  <button
                                    ...
                                    disabled={isPast || isBooked || !hasAmount || !isAvailable}
                                    style={{
                                      backgroundColor: slot.status === "booked" || isPast ? "#c9cfcfff" : isSelected ? "#374151" : slot.availabilityStatus !== "available" ? "#c9cfcfff" : "#FFFFFF",
                                      border: "2px solid #4949491A",
                                      cursor: !hasAmount || !isAvailable || isBooked || isPast ? "not-allowed" : "pointer",
                                      fontFamily: "Poppins",
                                      fontSize: "14px",
                                      transition: "all 0.2s ease",
                                      color: (slot.status === "booked" || isPast) ? "#000000" : isSelected ? "white" : "#000000",
                                    }}
                                    onMouseEnter={(e) => e.currentTarget.style.border = "1px solid #3DBE64"}
                                    onMouseLeave={(e) => e.currentTarget.style.border = "2px solid #4949491A"}
                                  >
