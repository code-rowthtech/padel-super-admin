...
import { getOwnerRegisteredClub, getActiveCourts, manualBookingByOwner } from '../../../../redux/thunks';
import { useDispatch, useSelector } from 'react-redux';
import { ButtonLoading, DataLoading, Loading } from '../../../../helpers/loading/Loaders';
const ManualBooking = () => {
    const dispatch = useDispatch();
    const { manualBookingLoading, ownerClubLoading, ownerClubData, activeCourtsLoading, activeCourtsData } = useSelector((state) => state.manualBooking)
    const slotTimes = activeCourtsData?.[0]?.slot?.[0]?.slotTimes || [];
    const courts = activeCourtsData?.[0]?.courts || []
    console.log(activeCourtsData?.[0]?.slot, 'slot')
    ...
    useEffect(() => {
        if (ownerClubData?.[0]?._id) {
            dispatch(getActiveCourts({ register_club_id: ownerClubData?.[0]?._id, day: selectedDay }));
        }
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            const slotsPayload = selectedCourts?.map(courtId => {
                ...
                const formattedBusinessHours = slotData?.businessHours?.map(bh => ({
                    time: bh.time,
                    day: bh.day
                })) || [];
                    ...
                    <Row className='mx-auto bg-white'>
                        <Col md={8} className="pt-3 rounded-3 px-4" >
                                        ...
                                        {dates?.map((d, i) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`calendar-day-btn border px-3 py-2 rounded ${isSelected ? "text-white" : "bg-light text-dark"}`}
                                                    style={{
                            ...
                            <div className="d-flex flex-wrap gap-2 mb-4">
                                {activeCourtsLoading ? <DataLoading height='20vh' /> :
                                    <>
                                        {slotTimes?.map((slot, i) => {
                                            ...
                                            return (
                                                <button
                                                    key={i}
                                                    className={`btn border-0 rounded-pill table-data px-4 ${isBooked ? 'bg-danger-subtle' : isPast ? 'bg-warning-subtle' : ''}`}
                                                    onClick={() => toggleTime(slot)}
                                                    title={`${isBooked ? 'Booked' : isPast ? 'Cannot book slot in past hours' : 'Book Now'}`}
                                ...
                                <div className="bg-white px-3">
                                    {activeCourtsLoading ? <DataLoading height='20vh' /> : <>
                                        {courts?.map((court) => (
                                            <div
                                                key={court._id}
                                                className={`d-flex justify-content-between align-items-center border-bottom py-3 mb-1 px-2 ${selectedCourts?.includes(court?._id) ? "bg-success-subtle rounded-pill" : ''}`}
                                            >
                                                {/* Left Image & Text */}
