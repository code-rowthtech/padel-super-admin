const Pagination = ({ totalRecords, defaultLimit, handlePageChange, currentPage }) => {
    ...
    if (totalPages <= visiblePages) {
        ...
        let startPage = Math.max(1, adjustedCurrentPage - Math.floor(visiblePages / 2));
    ...
    return (
        <div className="row py-3 w-100 d-flex algn-items-center">
            <div className="col-md-2 col-12 d-flex align-items-center justify-content-center justify-content-md-start mt-2 text-nowrap">
                <span className="ms-2 border p-1 ms-3 border-0 fw-bold">Total Records: {totalRecords}</span>
            </div>
            <div className="col-md-10 col-12 d-flex align-items-center justify-content-center justify-content-md-end">
                <div className="d-flex justify-content-end align-items-center">
                    <div className="d-flex align-items-center me-3">
                        <button
                            className="btn text-success mx-1 border-0 fw-bold bg-transparent"
                            onClick={() => handlePageChange(isZeroBased ? adjustedCurrentPage - 2 : adjustedCurrentPage - 1)}
                            ...
                            number === '...' ? (
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        background: number === adjustedCurrentPage ? '#3DBE64' : '',
                                        border: number === adjustedCurrentPage ? '1px solid #3DBE64' : '1px solid #3DBE64',
                                    }}
                                    onClick={() => handlePageChange(isZeroBased ? number - 1 : number)}>
                                    {number}
                                </button>
                            )
                        )}
                        ...
                        <button
                            className="btn text-success mx-1 border-0 fw-bold bg-transparent"
                            onClick={() => handlePageChange(isZeroBased ? adjustedCurrentPage : adjustedCurrentPage + 1)}
