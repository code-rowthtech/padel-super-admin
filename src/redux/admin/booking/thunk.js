...
export const getBookingByStatus = createAsyncThunk(
    ...
    async (params, { rejectWithValue }) => {
        try {
            ...
            const res = await getApi(`${Url.GET_BOOKING_BY_STATUS}?${buildQuery(params)}`);
            console.log({ res })
...
export const getBookingDetailsById = createAsyncThunk(
    ...
    async (params, { rejectWithValue }) => {
        try {
            const res = await getApi(`${Url.GET_BOOKING_DETAILS_BY_ID}?_id=${params?.id}`);
            // Destructure response data
            const { status, data, message } = res || {};
            if (status === 200 || '200') {
                return data;
...
export const updateBookingStatus = createAsyncThunk(
    "auth/updateBookingStatus", async (data, { rejectWithValue }) => {
        try {
            const res = await update(Url.UPDATE_BOOKING_STATUS, data);
