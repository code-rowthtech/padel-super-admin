...
export const getUserSlotPrice = createAsyncThunk(
  ...
  async (data, { rejectWithValue }) => {
    try {
      const res = await ownerApi.get(`${Url.UPDATE_SLOT_PRICE}?register_club_id=${data?.register_club_id}&day=${data?.day}`);
      if (res?.status === 200) {
        return res?.data;
      } else {
        showError(res?.data?.message || "Failed to get slot price");
        return rejectWithValue(res?.data?.message || "Failed to get slot price");
      }
    } catch (error) {
      showError(error);
    }
